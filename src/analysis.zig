...
pub fn makeDocumentScope(allocator: std.mem.Allocator, tree: Ast, zir: Zir) !DocumentScope {
...
fn makeScopeInternal(
    ...
    var scope = try context.scopes.addOne(allocator);
    const scope_idx = context.scopes.items.len - 1;

    scope.* = .{
        .range = nodeSourceRange(context.tree, @intCast(u32, src_node_offset)),
        .decls = std.StringHashMap(Declaration).init(allocator),
        .data = switch (context.tree.nodes.items(.tag)[@intCast(u32, src_node_offset)]) {
            .global_var_decl,
            .local_var_decl,
            .aligned_var_decl,
            .simple_var_decl,
            => b: {
                var decl = ast.varDecl(context.tree, @intCast(u32, src_node_offset)).?;
                std.debug.assert(decl.ast.init_node != 0);
                break :b .{ .container = decl.ast.init_node };
            },
            else => .{ .container = @intCast(u32, src_node_offset) },
        },
    };

    var decl_it = context.zir.declIterator(decl_inst);
    while (decl_it.next()) |item| {
        if (item.sub_index + 6 >= context.zir.extra.len) continue;

        const value_inst = context.zir.extra[item.sub_index + 6];
        switch (context.zir.instructions.items(.tag)[value_inst]) {
            .block_inline => {
                // TODO: This is an offset! Store offset in parent
                const pl_node = context.zir.instructions.items(.data)[value_inst].pl_node;
                const node_loc = @intCast(u32, src_node_offset + pl_node.src_node);
                try context.scopes.items[scope_idx].decls.put(try allocator.dupe(u8, item.name), .{ .ast_node = node_loc });

                var blk = context.zir.extraData(Zir.Inst.Block, pl_node.payload_index);
                const sub_inst_index = context.zir.extra[blk.end];
                const sub_tag = context.zir.instructions.items(.tag)[sub_inst_index];

                if (sub_tag != .extended) continue;
                var sub_ext = context.zir.instructions.items(.data)[sub_inst_index].extended;

                switch (sub_ext.opcode) {
                    .struct_decl, .union_decl, .enum_decl, .opaque_decl => {
                        try makeScopeInternal(allocator, context, src_node_offset + pl_node.src_node, sub_inst_index);
                    },
                    else => {},
                }
            },
