...
//! `zig build gen -- --generate-version-data 0.7.0` (requires an internet connection)
...
pub const builtins = [_]Builtin{
    ...
    .{
        ...
        \\
        \\A [pointer alignment safety check](https://ziglang.org/documentation/0.7.0/#Incorrect-Pointer-Alignment) is added to the generated code to make sure the pointer is aligned as promised.
        ,
        .arguments = &.{
    ...
    .{
        ...
        \\}
        \\```
        \\The result is a target-specific compile time constant. It is guaranteed to be less than or equal to [@sizeOf(T)](https://ziglang.org/documentation/0.7.0/#sizeOf).
        ,
        .arguments = &.{
            "comptime T: type",
        },
    ...
    .{
        ...
        .documentation =
        \\Performs [Type Coercion](https://ziglang.org/documentation/0.7.0/#Type-Coercion). This cast is allowed when the conversion is unambiguous and safe, and is the preferred way to convert between types, whenever possible.
        ,
        .arguments = &.{
            "comptime T: type",
            "expression",
    ...
    .{
        ...
        .documentation =
        \\`@asyncCall` performs an `async` call on a function pointer, which may or may not be an [async function](https://ziglang.org/documentation/0.7.0/#Async-Functions).
        \\
        \\The provided `frame_buffer` must be large enough to fit the entire function frame. This size can be determined with [@frameSize](https://ziglang.org/documentation/0.7.0/#frameSize). To provide a too-small buffer invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        \\
    ...
    .{
        ...
        \\ - `.Add` - for integers, twos complement wraparound addition. Also supports [Floats](https://ziglang.org/documentation/0.7.0/#Floats).
        \\ - `.Sub` - for integers, twos complement wraparound subtraction. Also supports [Floats](https://ziglang.org/documentation/0.7.0/#Floats).
        \\ - `.And` - bitwise and
        \\ - `.Nand` - bitwise nand
        \\ - `.Or` - bitwise or
        \\ - `.Xor` - bitwise xor
    ...
    .{
        ...
        \\
        \\For non [packed structs](https://ziglang.org/documentation/0.7.0/#packed-struct), this will always be divisible by `8`. For packed structs, non-byte-aligned fields will share a byte offset, but they will have different bit offsets.
        ,
        .arguments = &.{
            "comptime T: type",
    ...
    .{
        ...
        \\
        \\`operand` may be an [integer](https://ziglang.org/documentation/0.7.0/#Integers) or [vector](https://ziglang.org/documentation/0.7.0/#Vectors).
        \\
    ...
    .{
        ...
        \\
        \\If `integer` is known at [comptime](https://ziglang.org/documentation/0.7.0/#comptime), the return type is `comptime_int`. Otherwise, the return type is an unsigned integer with the minimum number of bits that can represent the bit count of the integer type.
        \\
    ...
    .{
        ...
        \\}
        \\```
        \\If you are using cmpxchg in a loop, [@cmpxchgWeak](https://ziglang.org/documentation/0.7.0/#cmpxchgWeak) is the better choice, because it can be implemented more efficiently in machine instructions.
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
    ...
    .{
        ...
        \\}
        \\```
        \\If you are using cmpxchg in a loop, the sporadic failure will be no problem, and `cmpxchgWeak` is the better choice, because it can be implemented more efficiently in machine instructions. However if you need a stronger guarantee, use [@cmpxchgStrong](https://ziglang.org/documentation/0.7.0/#cmpxchgStrong).
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
        \\`@typeInfo(@TypeOf(ptr)).Pointer.alignment` must be `>= @sizeOf(T).`
    ...
    .{
        ...
        \\
        \\If there is only one possible enum value, the resut is a `comptime_int` known at [comptime](https://ziglang.org/documentation/0.7.0/#comptime).
        ,
        .arguments = &.{
    ...
    .{
        ...
        \\
        \\ - [The Global Error Set](https://ziglang.org/documentation/0.7.0/#The-Global-Error-Set)
        \\ - [Error Set Type](https://ziglang.org/documentation/0.7.0/#Error-Set-Type)
        \\ - [Error Union Type](https://ziglang.org/documentation/0.7.0/#Error-Union-Type)
    ...
    .{
        ...
        .documentation =
        \\Converts an error value from one error set to another error set. Attempting to convert an error which is not in the destination error set results in safety-protected [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime T: DestType",
            "value: anytype",
    ...
    .{
        ...
        \\
        \\This function can be called from a [comptime](https://ziglang.org/documentation/0.7.0/#comptime) block to conditionally export symbols. When `target` is a function with the C calling convention and `options.linkage` is `Strong`, this is equivalent to the `export` keyword used on a function:
        \\
        \\```zig
        \\comptime {
    ...
    .{
        ...
        \\
        \\If the integer part of the floating point number cannot fit in the destination type, it invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "float: anytype",
    ...
    .{
        ...
        .snippet = "@frame()",
        .documentation =
        \\This function returns a pointer to the frame for a given function. This type can be [coerced](https://ziglang.org/documentation/0.7.0/#Type-Coercion) to `anyframe->T` and to `anyframe`, where `T` is the return type of the function in scope.
        \\
        \\This function does not mark a suspension point, but it does cause the function in scope to become an [async function](https://ziglang.org/documentation/0.7.0/#Async-Functions).
        ,
        .arguments = &.{},
    },
    .{
        ...
        .documentation =
        \\This function returns the frame type of a function. This works for [Async Functions](https://ziglang.org/documentation/0.7.0/#Async-Functions) as well as any function without a specific calling convention.
        \\
        \\This type is suitable to be used as the return type of [async](https://ziglang.org/documentation/0.7.0/#Async-and-Await) which allows one to, for example, heap-allocate an async function frame:
        \\
        \\```zig
        \\const std = @import("std");
    ...
    .{
        ...
        \\
        \\This function is typically used in conjunction with [@asyncCall](https://ziglang.org/documentation/0.7.0/#asyncCall).
        ,
        .arguments = &.{},
    },
    .{
        ...
        .documentation =
        \\Returns whether or not a [struct](https://ziglang.org/documentation/0.7.0/#struct), [enum](https://ziglang.org/documentation/0.7.0/#enum), or [union](https://ziglang.org/documentation/0.7.0/#union) has a declaration matching `name`.
        \\
        \\```zig
        \\const std = @import("std");
        \\const expect = std.testing.expect;
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to another integer while keeping the same numerical value. Attempting to convert a number which is out of range of the destination type results in safety-protected [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        \\
        \\If `T` is `comptime_int`, then this is semantically equivalent to [Type Coercion](https://ziglang.org/documentation/0.7.0/#Type-Coercion).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "int: anytype",
    ...
    .{
        ...
        .documentation =
        \\Converts an integer into an [enum](https://ziglang.org/documentation/0.7.0/#enum) value.
        \\
        \\Attempting to convert an integer which represents no value in the chosen enum type invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
    ...
    .{
        ...
        .documentation =
        \\Converts from the integer representation of an error into [The Global Error Set](https://ziglang.org/documentation/0.7.0/#The-Global-Error-Set) type.
        \\
        \\It is generally recommended to avoid this cast, as the integer representation of an error is not stable across source code changes.
        \\
        \\Attempting to convert an integer that does not correspond to any error results in safety-protected [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to the closest floating point representation. To convert the other way, use [@floatToInt](https://ziglang.org/documentation/0.7.0/#floatToInt). This cast is always safe.
        ,
        .arguments = &.{
            "comptime DestType: type",
            "int: anytype",
    ...
    .{
        ...
        .documentation =
        \\Converts an integer to a [pointer](https://ziglang.org/documentation/0.7.0/#Pointers). To convert the other way, use [@ptrToInt](https://ziglang.org/documentation/0.7.0/#ptrToInt).
        \\
        \\If the destination pointer type does not allow address zero and `address` is zero, this invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "address: usize",
    ...
    .{
        ...
        \\
        \\[Optional Pointers](https://ziglang.org/documentation/0.7.0/#Optional-Pointers) are allowed. Casting an optional pointer which is [null](https://ziglang.org/documentation/0.7.0/#null) to a non-optional pointer invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "comptime DestType: type",
            "value: anytype",
    ...
    .{
        ...
        \\ - `fn()`
        \\ - `?fn()`
        \\To convert the other way, use [@intToPtr](https://ziglang.org/documentation/0.7.0/#intToPtr)
        ,
        .arguments = &.{
            "value: anytype",
        },
    ...
    .{
        ...
        .documentation =
        \\Constructs a new [vector](https://ziglang.org/documentation/0.7.0/#Vectors) by selecting elements from `a` and `b` based on `mask`.
        \\
        ...
        \\
        \\`E` must be an [integer](https://ziglang.org/documentation/0.7.0/#Integers), [float](https://ziglang.org/documentation/0.7.0/#Floats), [pointer](https://ziglang.org/documentation/0.7.0/#Pointers), or `bool`. The mask may be any vector length, and its length determines the result length.
        ,
        .arguments = &.{
            "comptime E: type",
    ...
    .{
        ...
        \\
        \\This size may contain padding bytes. If there were two consecutive T in memory, this would be the offset in bytes between element at index 0 and the element at index 1. For [integer](https://ziglang.org/documentation/0.7.0/#Integers), consider whether you want to use `@sizeOf(T)` or `@typeInfo(T).Int.bits`.
        \\
        \\This function measures the size at runtime. For types that are disallowed at runtime, such as `comptime_int` and `type`, the result is `0`.
        ,
        .arguments = &.{
    ...
    .{
        ...
        \\}
        \\```
        \\`scalar` must be an [integer](https://ziglang.org/documentation/0.7.0/#Integers), [bool](https://ziglang.org/documentation/0.7.0/#Primitive-Types), [float](https://ziglang.org/documentation/0.7.0/#Floats), or [pointer](https://ziglang.org/documentation/0.7.0/#Pointers).
        ,
        .arguments = &.{
            "comptime len: u32",
            "scalar: anytype",
    ...
    .{
        ...
        .documentation =
        \\Transforms a [vector](https://ziglang.org/documentation/0.7.0/#Vectors) into a scalar value by performing a sequential horizontal reduction of its elements using the specified specified operator `op`.
        \\
        \\Not every operator is available for every vector element type:
        \\
        \\ - `.And`, `.Or`, `.Xor` are available for `bool` vectors,
        \\ - `.Min`, `.Max`, `.Add`, `.Mul` are available for [floating point](https://ziglang.org/documentation/0.7.0/#Floats) vectors,
        \\ - Every operator is available for [integer](https://ziglang.org/documentation/0.7.0/#Integers) vectors.
    ...
    .{
        ...
        \\
        \\Supports [Floats](https://ziglang.org/documentation/0.7.0/#Floats) and [Vectors](https://ziglang.org/documentation/0.7.0/#Vectors) of floats, with the caveat that
    ...
    .{
        ...
        \\
        \\If the enum is non-exhaustive and the tag value does not map to a name, it invokes safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior).
        ,
        .arguments = &.{
            "value: anytype",
        },
    ...
    .{
        ...
        \\
        \\The following produces safety-checked [Undefined Behavior](https://ziglang.org/documentation/0.7.0/#Undefined-Behavior):
        \\
        \\```zig
    ...
    .{
        ...
        .documentation =
        \\This function is the inverse of [@typeInfo](https://ziglang.org/documentation/0.7.0/#typeInfo). It reifies type information into a `type`.
        \\
        ...
        \\ - `void`
        \\ - `bool`
        \\ - [Integers](https://ziglang.org/documentation/0.7.0/#Integers) - The maximum bit count for an integer type is `65535`.
        \\ - [Floats](https://ziglang.org/documentation/0.7.0/#Floats)
        \\ - [Pointers](https://ziglang.org/documentation/0.7.0/#Pointers)
        \\ - `comptime_int`
        \\ - `comptime_float`
        \\ - `@TypeOf(undefined)`
        \\ - `@TypeOf(null)`
        \\ - [Arrays](https://ziglang.org/documentation/0.7.0/#Arrays)
        \\ - [Optionals](https://ziglang.org/documentation/0.7.0/#Optionals)
        \\ - [Error Set Type](https://ziglang.org/documentation/0.7.0/#Error-Set-Type)
        \\ - [Error Union Type](https://ziglang.org/documentation/0.7.0/#Error-Union-Type)
        \\ - [Vectors](https://ziglang.org/documentation/0.7.0/#Vectors)
        \\ - [opaque](https://ziglang.org/documentation/0.7.0/#opaque)
        \\ - [@Frame](https://ziglang.org/documentation/0.7.0/#Frame)
        \\ - `anyframe`
        \\ - [struct](https://ziglang.org/documentation/0.7.0/#struct)
        \\ - [enum](https://ziglang.org/documentation/0.7.0/#enum)
        \\ - [Enum Literals](https://ziglang.org/documentation/0.7.0/#Enum-Literals)
        \\ - [union](https://ziglang.org/documentation/0.7.0/#union)
        \\For these types, `@Type` is not available:
        \\
        \\ - [Functions](https://ziglang.org/documentation/0.7.0/#Functions)
        \\ - BoundFn
        ,
        .arguments = &.{
    ...
    .{
        ...
        .snippet = "@TypeOf(${1:...})",
        .documentation =
        \\`@TypeOf` is a special builtin function that takes any (nonzero) number of expressions as parameters and returns the type of the result, using [Peer Type Resolution](https://ziglang.org/documentation/0.7.0/#Peer-Type-Resolution).
        \\
    ...
    .{
        ...
        .documentation =
        \\This is the same thing as [union](https://ziglang.org/documentation/0.7.0/#union) initialization syntax, except that the field name is a [comptime](https://ziglang.org/documentation/0.7.0/#comptime)-known value rather than an identifier token.
        \\
        \\`@unionInit` forwards its [result location](https://ziglang.org/documentation/0.7.0/#Result-Location-Semantics) to `init_expr`.
        ,
        .arguments = &.{
            "comptime Union: type",
