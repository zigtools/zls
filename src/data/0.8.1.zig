...
pub const builtins = [_]Builtin{
    ...
    .{
        ...
        \\}
        \\```
        \\If you are using cmpxchg in a loop, [@cmpxchgWeak](https://ziglang.org/documentation/0.8.1/#cmpxchgWeak) is the better choice, because it can be implemented more efficiently in machine instructions.
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
    ...
    .{
        ...
        \\}
        \\```
        \\If you are using cmpxchg in a loop, the sporadic failure will be no problem, and `cmpxchgWeak` is the better choice, because it can be implemented more efficiently in machine instructions. However if you need a stronger guarantee, use [@cmpxchgStrong](https://ziglang.org/documentation/0.8.1/#cmpxchgStrong).
        \\
        \\`T` must be a pointer, a `bool`, a float, an integer or an enum.
        \\
        \\`@typeInfo(@TypeOf(ptr)).Pointer.alignment` must be `>= @sizeOf(T).`
    ...
    .{
        ...
        .documentation =
        \\Constructs a new [vector](https://ziglang.org/documentation/0.8.1/#Vectors) by selecting elements from `a` and `b` based on `mask`.
        \\
        ...
        \\
        \\`E` must be an [integer](https://ziglang.org/documentation/0.8.1/#Integers), [float](https://ziglang.org/documentation/0.8.1/#Floats), [pointer](https://ziglang.org/documentation/0.8.1/#Pointers), or `bool`. The mask may be any vector length, and its length determines the result length.
        ,
        .arguments = &.{
            "comptime E: type",
    ...
    .{
        ...
        \\
        \\This size may contain padding bytes. If there were two consecutive T in memory, this would be the offset in bytes between element at index 0 and the element at index 1. For [integer](https://ziglang.org/documentation/0.8.1/#Integers), consider whether you want to use `@sizeOf(T)` or `@typeInfo(T).Int.bits`.
        \\
        \\This function measures the size at runtime. For types that are disallowed at runtime, such as `comptime_int` and `type`, the result is `0`.
        ,
        .arguments = &.{
    ...
    .{
        ...
        \\}
        \\```
        \\`scalar` must be an [integer](https://ziglang.org/documentation/0.8.1/#Integers), [bool](https://ziglang.org/documentation/0.8.1/#Primitive-Types), [float](https://ziglang.org/documentation/0.8.1/#Floats), or [pointer](https://ziglang.org/documentation/0.8.1/#Pointers).
        ,
        .arguments = &.{
            "comptime len: u32",
            "scalar: anytype",
    ...
    .{
        ...
        .documentation =
        \\Transforms a [vector](https://ziglang.org/documentation/0.8.1/#Vectors) into a scalar value by performing a sequential horizontal reduction of its elements using the specified operator `op`.
        \\
