...
/// A modified version of tree.tokenSlice to return null if the tokenizer encounters an unexpected token
...
pub fn tokenSlice(tree: Ast, token_index: Ast.TokenIndex) ?[]const u8 {
    ...
    if (token.tag != token_tag) return null; // assert(token.tag == token_tag);
