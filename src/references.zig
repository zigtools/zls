...
pub fn labelReferences(
    ...
    if (include_decl) {
        ...
        try locations.append(allocator, .{
            .uri = handle.uri(),
            .range = offsets.tokenToRange(handle.tree, first_tok, encoding) catch return locations,
        });
    }
    ...
    while (curr_tok < last_tok - 2) : (curr_tok += 1) {
        ...
        try locations.append(allocator, .{
            .uri = handle.uri(),
            .range = offsets.tokenToRange(handle.tree, curr_tok + 2, encoding) catch continue,
        });
    }
...
fn symbolReferencesInternal(
    ...
    switch (node_tags[node]) {
        ...
        .identifier => blk: {
            const child = (try analysis.lookupSymbolGlobal(store, arena, handle, tree.getNodeSource(node), starts[main_tokens[node]])) orelse break :blk;
