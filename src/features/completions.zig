...
fn typeToCompletion(builder: *Builder, ty: Analyser.Type) error{ OutOfMemory, WriteFailed }!void {
...
fn declToCompletion(builder: *Builder, decl_handle: Analyser.DeclWithHandle) error{ OutOfMemory, WriteFailed }!void {
...
fn completeGlobal(builder: *Builder) error{ OutOfMemory, WriteFailed }!void {
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
...
fn completeFieldAccess(builder: *Builder, loc: offsets.Loc) error{ OutOfMemory, WriteFailed }!void {
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
...
fn completeDot(builder: *Builder, loc: offsets.Loc) error{ OutOfMemory, WriteFailed }!void {
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
...
pub fn completionAtIndex(
    ...
    source_index: usize,
) error{ OutOfMemory, WriteFailed }!?types.CompletionList {
...
) error{ OutOfMemory, WriteFailed }![]Analyser.Type {
    const tracy_zone = tracy.trace(@src());
    defer tracy_zone.end();
