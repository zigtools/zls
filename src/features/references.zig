    ...
    fn referenceNode(self: *const Context, tree: Ast, node: Ast.Node.Index) error{OutOfMemory}!void {
        ...
        switch (tree.nodeTag(node)) {
            ...
            .field_access => {
                ...
                const lhs = try builder.analyser.resolveExprOfNode(.of(lhs_node, handle)) orelse return;
                const deref_lhs = try builder.analyser.resolveDerefExpr(lhs) orelse lhs;
            ...
            => {
                ...
                for (struct_init.ast.fields) |value_node| { // the node of `value` in `.name = value`
                    ...
                    const lookup = try builder.analyser.lookupSymbolEnumLiteral(handle, name, node, &.{}) orelse continue;
