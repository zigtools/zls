...
fn generalReferencesHandler(server: *Server, writer: anytype, id: types.RequestId, req: GeneralReferencesRequest) !void {
    ...
    const doc_position = try offsets.documentPosition(handle.document, req.position(), server.offset_encoding);
    const pos_context = try analysis.documentPositionContext(&server.arena, handle.document, doc_position);

    const decl = switch (pos_context) {
        .var_access => try server.getSymbolGlobal(doc_position.absolute_index, handle),
        .field_access => |range| try server.getSymbolFieldAccess(handle, doc_position, range),
        .label => try getLabelGlobal(doc_position.absolute_index, handle),
        else => null,
    } orelse return try respondGeneric(writer, id, null_result_response);
