...
pub fn main(init: process.Init.Minimal) !void {
    ...
    var watch = false;
    var check_step_only = false; // Only for ZLS
    // Cut out for ZLS
    ...
    var run: Run = .{
        ...
        .summary = summary orelse if (watch) .line else .failures,
    };
    defer {
    ...
    prepare(arena, builder, targets.items, &run, graph.random_seed, check_step_only) catch |err| switch (err) {
    ...
    rebuild: while (true) : (if (run.error_style.clearOnUpdate()) {
        ...
        const caption = std.fmt.bufPrint(&caption_buf, "watching {d} directories, {d} processes", .{
            w.fs_watch.dir_count, countSubProcesses(run.step_stack.keys()),
        }) catch &caption_buf;
...
const Watch = struct {
    ...
    fn wait(w: *Watch, gpa: Allocator, timeout: std.Build.Watch.Timeout) !std.Build.Watch.WaitResult {
        ...
        const io_timeout: std.Io.Timeout = switch (timeout) {
            .none => .none,
            .ms => |d| .{
                .duration = .{
                    .clock = std.Io.Clock.real,
                    .raw = std.Io.Duration.fromMilliseconds(@intCast(d)),
                },
            },
        };
        waitTimeout(&w.manual_event, w.io, io_timeout) catch |err| switch (err) {
...
fn runStepNames(
...

    {
        ...
        if (run.watch) {
            for (initial_set.items) |step| {
                const step_id: u32 = @intCast(run.step_stack.getIndex(step).?);
                // missing fields:
                // - result_error_msgs
                // - result_stderr
...
fn makeStep(
    ...
    if (run.watch) {
        const step_id: u32 = @intCast(run.step_stack.getIndex(s).?);
        // missing fields:
        // - result_error_msgs
        // - result_stderr
...
fn extractBuildInformation(
    ...
    const step_dependencies = &run.step_stack;
    ...
    prepare(gpa, b, step_names, run, b.graph.random_seed, false) catch |err| switch (err) {
        error.DependencyLoopDetected => {
            // Perhaps in the future there could be an Advanced Options flag
            // such as --debug-build-runner-leaks which would make this code
            // return instead of calling exit.
