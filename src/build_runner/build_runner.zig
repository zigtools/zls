...
fn prepare(
    ...
    for (starting_steps) |s| {
        constructGraphAndCheckForDependencyLoop(gpa, b, s, unpopulated_step_stack, rand) catch |err| switch (err) {
...
fn runSteps(
    ...
    for (steps_stack.keys()) |step| {
        ...
        group.async(io, workerMakeOneStep, .{
            &group, b, steps_stack, step, step_prog, run,
        });
    }
...
fn workerMakeOneStep(
    ...
    handle_result: {
        ...
        for (s.dependants.items) |dep| {
            group.async(io, workerMakeOneStep, .{
                group, b, steps_stack, dep, prog_node, run,
            });
        }
    }
